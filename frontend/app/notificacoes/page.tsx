...
export default function NotificacoesPage() {
  const [notificacoes, setNotificacoes] = useState([
    { id: 1, titulo: 'Pedido realizado com sucesso!', msg: 'O pedido #882 foi processado.', data: 'Há 3 horas atrás', lida: false },
    { id: 2, titulo: 'Pedido pendente!', msg: 'Um novo pedido aguarda sua revisão.', data: 'Há 13 horas atrás', lida: false },
    { id: 3, titulo: 'Pedido enviado!', msg: 'A remessa saiu para entrega.', data: 'Há 1 dia atrás', lida: true },
  ]);
        ...
        <div className="max-w-4xl mx-auto mt-10">
          ...
          <header className="mb-8 flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 mt-1">Histórico completo de notificações do sistema.</p>
            </div>
          ...
          <div className="space-y-1 border border-gray-800 rounded-2xl overflow-hidden bg-[#161b22]">
            {notificacoes.map((n) => (
              <div 
                key={n.id} 
                className={`p-6 border-b border-gray-800/50 last:border-0 transition-all hover:bg-[#1c2128] group ${
                  n.lida ? 'opacity-60' : 'bg-[#161b22]'
                }`}
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => marcarLida(n.id)}>
                    {/* Ícone de sino lateral igual ao protótipo */}
                    <div className={`${n.lida ? 'text-gray-600' : 'text-gray-400'}`}>
                       <Icons.Bell />
                    </div>
                    <div>
                      <h3 className={`text-lg font-semibold ${n.lida ? 'text-gray-400' : 'text-gray-100'}`}>
                        {n.titulo}
                      </h3>
