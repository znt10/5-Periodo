  ...
  return (
    <div className="flex min-h-screen bg-[#0d1117] text-gray-100 font-sans antialiased">
      ...
      <main className="flex-1 lg:ml-64 p-8 md:p-12 transition-all">
        ...
        {/* Search Bar Padronizada (Padrão Pedidos Hoje) */}
        ...
        <div className="max-w-5xl">
          ...
          <div className="grid gap-5">
            {secoes.map((secao, index) => (
              <button 
                key={index}
                className="w-full bg-[#161b22] border border-gray-800 rounded-[28px] p-7 flex items-center justify-between hover:border-blue-500/30 hover:bg-[#1c222a] transition-all group shadow-2xl active:scale-[0.985]"
              >
                <div className="flex items-center gap-8">
                  <div className="bg-[#0d1117] p-5 rounded-[22px] text-blue-500 border border-gray-800 group-hover:border-blue-500/20 group-hover:scale-105 transition-all shadow-inner">
                    {secao.icon}
                  </div>
                  <div>
                ...
                <div className="bg-[#0d1117] p-3 rounded-full text-gray-700 border border-gray-800 group-hover:text-blue-500 group-hover:border-blue-500/30 group-hover:translate-x-1 transition-all">
                  <Icons.ChevronRight />
                </div>
              </button>
            ))}
